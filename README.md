...
目标：让浏览器扩展在同一套 UI 中自由切换后端服务模式和独立运行模式。未部署后端时，插件可直接使用本地存储和用户自行配置的模型服务完成素材管理与分析。

### 运行与数据规则

- [ ] 在插件设置中提供“独立运行”和“后端服务”两种模式，并清晰展示当前模式、连接状态和存储占用。
- [ ] 未配置后端时使用独立运行模式；切换模式不应丢失当前模式下的数据。
- [ ] 本地资料库与后端资料库默认相互隔离，不在模式切换时自动复制或合并数据。
- [ ] 提供显式的数据迁移入口，支持“本地迁移到后端”和“后端迁移到本地”，迁移前展示数量、冲突和预计占用空间。
- [ ] 后端临时不可用时不静默切换数据源，避免用户误以为资料丢失；后续可增加离线待同步队列。

### 实施路线

- [ ] 抽象 `StorageAdapter`、`AnalysisAdapter` 等统一接口，解除 Popup、Content Script 和结果弹窗对 `/api` 及 `serverUrl` 的直接依赖。
- [ ] 保留现有后端适配器，补充回归测试，确保当前上传、分析、保存、搜索和导出流程不受影响。
- [ ] 实现本地存储适配器：使用 IndexedDB 保存元数据、分析结果和任务状态，使用 OPFS 保存图片、视频等大文件，使用 `chrome.storage.local` 保存模式、模型和界面设置。
- [ ] 区分临时分析记录与已保存灵感，关闭分析弹窗后保留会话记录，但只有用户主动保存后才进入本地资料库。
- [ ] 增加插件内本地工作台，覆盖时间轴、搜索、详情、导出、导入、备份和存储清理等核心能力。
- [ ] 支持用户自带 API Key（BYOK），优先接入当前默认模型，并预留其他图片、视频理解模型供应商适配接口。
- [ ] 将视频分析改造成可恢复的持久化任务，支持 Popup 关闭后继续执行、重新打开后恢复进度与生成状态。
- [ ] 实现本地与后端之间的迁移服务，包括去重、冲突处理、失败重试、迁移报告和可中止操作。
- [ ] 增加独立模式的安全提示与备份机制，明确浏览器扩展卸载或站点数据被清理可能导致本地资料丢失。
