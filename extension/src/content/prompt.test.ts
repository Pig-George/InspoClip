...
describe("content prompt helpers", () => {
  ...
  test("extracts a refreshed bilingual prompt from an image analysis response", () => {
    expect(extractPromptFromImageAnalysis({
      terms: ["glass card"],
      prompt: { en: "Refreshed English prompt", zh: "更新后的中文提示词" }
    })).toEqual({ en: "Refreshed English prompt", zh: "更新后的中文提示词" })
    expect(extractPromptFromImageAnalysis({ prompt: { en: "" } })).toBeNull()
  })
  ...
  test("keeps a prompt regeneration active while its modal is closed and reopened", async () => {
    ...
    // The same history entry is used when the user reopens its collapsed modal.
